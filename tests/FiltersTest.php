...
final class FiltersTest extends TestCase
{
    private function requestWith(array $opts = [])
    {
        ...
        return new class($opts) implements \CodeIgniter\HTTP\RequestInterface {
            ...
            public function getServer($index=null){return null;} public function getEnv($index=null){return null;}
            public function getBody(){return '';} public function setBody($body){return $this;}
            public function getHeader($name){return null;} public function headers(){return [];} public function hasHeader($name){return false;}
            public function getProtocolVersion(){return '1.1';} public function setProtocolVersion($version){return $this;}
            public function getPost($index=null){return null;} public function getPostGet($index=null){return null;}
            public function getJSON($assoc=false){return null;} public function getRawInput(){return [];} public function getUserAgent(){return '';}            
        };
    }
    ...
    public function testRateLimitHeadersAndBlocking(): void
    {
        ...
        $config->limitModelClass = new class {
            private static array $rows = []; private static int $auto=1; private static $lastInsertId; private array $w=[];
            public function where(array $w){ $this->w=$w; return $this; }
            public function get(){ $row=null; foreach(self::$rows as $r){ $ok=true; foreach(($this->w??[]) as $k=>$v){if(($r[$k]??null)!==$v){$ok=false;break;}} if($ok){$row=$r; break;}} return new class($row){ private $r; public function __construct($r){$this->r=$r;} public function getRowArray(){return $this->r;} }; }
            public function insert(array $row){ $row['id']=self::$auto++; self::$rows[$row['id']]=$row; self::$lastInsertId=$row['id']; }
            public function getInsertID(){ return self::$lastInsertId; }
            public function update($id, array $data){ self::$rows[$id] = array_merge(self::$rows[$id], $data); }
